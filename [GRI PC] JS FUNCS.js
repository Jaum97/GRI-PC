...
function refreshCommons(obj){
  //let {productsCommonSecReg,productsCommonSecRegCon,productsCommonSecRegConSeg,productsSectorOn,productsRegionOn,productsCountryOn,productsSegmentOn} = obj;
...
function validateFilters(filterType, classesValueArray) {
  let obj = preferenceCenterObject;
  let {productsCommonSecReg:arr1,productsCommonSecRegCon:arr2,productsCommonSecRegConSeg:arr3,filterSectorOn:filterSec,filterSegmentOn:filterSeg} = obj;
  let blank = [];
  ...
  if (filterType == "Sector") {
    if ((classesValueArray === blank) || (arr2.length)) {
    ...
    } else if ((arr3.length) || (!filterSeg.length)) {
      clearInnerHTML("activeSegmentFiltersHere");
      clearInnerHTML("activeSegmentButtonFiltersHere");
    } else if (arr1.length) {
...
function clearAll(obj){
  ...
  createFilterOption(matrix, 'testRegionFilterLocation', arrayNoDups(returnFieldifMatch(matrix, '4', '12', 'active')), 'Region', 'buttonRegionOff', 'buttonRegionOn', 'no');
  createFilterOption(matrix, 'testSectorFilterLocation', arrayNoDups(returnFieldifMatch(matrix, '3', '12', 'active')), 'Sector', 'buttonSectorOff', 'buttonSectorOn', 'no');
  createFilterOption(matrix, 'testCountryFilterLocation', arrayBlank, 'Country', 'buttonCountryOff', 'buttonCountryOn', 'no');
  createFilterOption(matrix, 'testSegmentFilterLocation', arrayBlank, 'Segment', 'buttonSegmentOff', 'buttonSegmentOn', 'no');
  return false;
...
function searchBox(input) {
  ...
  let matchedArray = [];
  for (let i = obj.trinity.length; i--;(obj.trinity[i].toUpperCase().indexOf(toMatch) > -1) ? matchedArray.push(obj.trinity[i]) : false){}
  clearInnerHTML("debugPrints0");
  (matchedArray.length) ? createProductBlocky(obj.neo, matchedArray, 'debugPrints0'): targetPosition.innerHTML = "SORRY NO PRODUCTS FOUND";
}
